...
function randomPlayers(
  ...
  for (let i = 1; i <= playersAmount; i++) {
    ...
    if (donAmount >= 1) {
      ...
      mafiaAmount--
    } else if (sheriffAmount >= 1) {
      playersList[playersWithoutRoles[playerId]] = 'Шериф'
      sheriffAmount--
...
export const Players: React.FC = () => {
  const [shouldRerollPlayers, setShouldRerollPlayers] = useState<boolean>(false)
  ...
  let sheriffAmount = useSelector((state: RootState) => state.sheriffAmount)
  ...
  const slotsList = useMemo(
    () => randomSlots(playersAmount),
    /* eslint-disable-next-line react-hooks/exhaustive-deps */
    [playersAmount, shouldRenderSlots]
  )
    ...
    return playersList.map((player, index) => {
      return (
        <Player
          key={String(index)}
          number={index + 1}
          role={playersList[index]}
          slot={shouldRenderSlots ? slotsList[index] : null}
    ...
    <Section>
      <div className={classes.players}>
        <div className={classes.playersControls}>
          <button
            className={classes.playersControlsButton}
            onClick={() => setShouldRerollPlayers(!shouldRerollPlayers)}
          >
            Генерація ролей
          </button>
          {!shouldRenderSlots ? (
            <span className={classes.playersControlsCheckButton}>
              <span className={classes.playersControlsCheckButtonLabel}>
                Зміна слотів
              </span>
              <button
                className={classes.playersControlsCheckButtonSwitch}
                onClick={() => setShouldRenderSlots(!shouldRenderSlots)}
              >
                <span
                  className={classes.playersControlsCheckButtonSwitchTrack}
                />
              </button>
            </span>
          ) : (
            <span className={classes.playersControlsCheckButton}>
              ...
              <button
                className={
                  classes.playersControlsCheckButtonSwitch +
                  ' ' +
                  slotsRenderActive
                }
                onClick={() => setShouldRenderSlots(!shouldRenderSlots)}
              >
                <span
                  className={
                    classes.playersControlsCheckButtonSwitchTrack +
                    ' ' +
                    slotsRenderActive
                  }
                />
        ...
        <div className={classes.flexWrapper}>
          <div className={classes.flexWrapperItem}>
            <ul className={classes.playersList}>{renderPlayers()}</ul>
          </div>
          <div className={classes.flexWrapperItem}>
            <OptionsList />
          </div>
