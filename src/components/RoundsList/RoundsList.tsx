...
export const RoundsList: React.FC = () => {
  ...
  const roundsKeys = useSelector((state: RootState) => Object.keys(state.rounds))

  function renderRounds() {
    return roundsList.map((_, index) => {
      return <Round key={String(index)} notes={[...roundsList[index]]} round={Number(roundsKeys[index])} />
    })
  }
